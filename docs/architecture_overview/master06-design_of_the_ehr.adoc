...
While any of these forms is valid, a reverse domain name (e.g. `uk.nhs.ehr1`, `se.regionstockholm.openehr`) is the commonly recommended choice where the organisation legally responsible for the system has a stable domain, since it is globally unique, human-readable, and orders from the most general to the most specific part. A UUID/GUID or ISO OID is equally suitable where no stable domain applies or an opaque identifier is preferred. Because the `_system_id_` becomes embedded in the version identifiers of committed -- and possibly signed -- content, a globally unique form should be settled on before data is shared or federated, since it cannot easily be changed afterwards. Note that the `_system_id_` identifies the _logical_ system responsible for the data, and is distinct both from the physical server host name (usually a forward domain name) and from any query-federation node identifier.
