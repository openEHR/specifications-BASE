...
The {openehr_base_types}[Base Types specification] defines the most basic concepts internal to {openehr_foundation_types}, used by all other packages. It is comprised of the `definitions`, `identification`, `terminology` and  `measurement` sub-packages. The semantics defined in these packages allow all other models to use identifiers and to have access to knowledge services like terminology and other reference data.
...
The {openehr_base_bmm}[BMM specification] defines a generic meta-model, suitable for formally expressing other models, including those of openEHR. It is roughly an equivalent of UML's XMI, but fixes various problems with the latter around generic (template) types, while being vastly less complex and obscure. BMM models can be expressed in the ODIN or any other regular object syntax (JSON etc), and conveniently edited by hand. BMM files are used within tools such as the {openehr_awb}[openEHR ADL Workbench].
...
image::{rm_uml_export_dir}/diagrams/RM-packages.svg[id=rm_packages, align="center"]
...
[.text-center]
.Structure of the ADL 1.4 version `org.openehr.am` package
image::{am_uml_export_dir}/diagrams/AOM-packages.png[id=aom_2_packages, align="center"]

The package structure of the version 1.4 form of the AM is shown below.

[.text-center]
.Structure of the ADL 2 `org.openehr.am` package
image::{am_uml_export_dir}/AOM1.4/diagrams/AOM-packages.png[id=aom_14_packages, align="center"]
...
image::diagrams/openehr_block_diagram.png["openEHR_packages_and_languages",align="center",width="75%"]
