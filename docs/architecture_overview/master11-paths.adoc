...
The possible values for `top_level_structure_locator` are a subset of the attribute names on the class `EHR`, visible in the `ehr` package, namely `_compositions_`, `_ehr_status_`, `_ehr_access_`, and `_directory_`.
