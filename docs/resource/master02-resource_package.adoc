...
Authored resources contain natural language elements, and are therefore created in some original language, recorded in the `_original_language_` attribute of the `AUTHORED_RESOURCE` class. Information about translations is included in the translations attribute, which allows for one or more sets of translation details to be recorded. A resource is translated by doing the following:
