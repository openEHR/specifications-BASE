...
Iteration in EL is performed over container and interval objects, i.e. variables or manifest constants. The general approach is to use built-in iterators of the container and interval types, i.e. `do_all`, `do_if`, `do_while` and `do_until` as defined earlier.
