...
The openEHR Expression Language (EL) syntax can be understood in a number of layers. The core of the syntax defines _expressions_, i.e. value-returning entities. Such expressions may be used in isolation within some larger context in an openEHR system (e.g. Plan or Guideline), with the connection of variables to real-world data being determined implicitly, as dscribed above.

The next layer includes non-expression elements, allowing explicit declarations, assignments and control structures, including rules. Together with expressions, these are known as _statements_. A collection of such statements may be used within another context in the same way as for bare expressions.
