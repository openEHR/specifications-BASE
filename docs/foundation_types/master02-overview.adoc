...
Within all object-oriented and most other modern programming and modelling environment exists an ultimate ancestor type to which all others conform. In this specification, we assume the type `Any`, which defines a bare minimum of operations, and stands for the real `Any` or `Object` type found in real technologies.
