...
The following UML diagram shows the semantically important part of the BMM, defining classes, types and their relations.
...
[.text-center]
.Enumeration Example
image::images/awb_enumeration.png[id=awb_enumeration, align="center", width="60%"]
...
image::images/awb_generic_class.png[id=awb_generic_class, align="center", width="45%"]

=== Generic Effective Classes

Generic classes give rise to two kinds of generic types: open, and derived, or 'effective'. The latter kind have one or more formal parameters from the class definition substituted with an actual type. From the `DV_INTERVAL<T:DV_ORDERED>` class the open type `DV_INTERVAL<T>` and numerous effective types such as `DV_INTERVAL<DV_DATE_TIME>` are possible, based on actual types that inherit from `DV_ORDERED`. As a convenience in the BMM modelling environment, an'effective' form of `BMM_GENERIC_CLASS` is synthesised to correspond to such concrete types. In the synthesised from, properties whose types were formal generic parameters are changed to be of the concrete subsituted type. The following shows the example visualised.

[.text-center]
.BMM generic effective class
image::images/awb_class_generic_effective.png[id=awb_class_generic_effective, align="center", width="90%"]

== Inheritance

The BMM supports single and multiple inheritance, although it does not distinguish between different types of inheritance relation as some programming languages do. Inheritance is formally defined to be between a class definition (an instance of `BMM_CLASS`) and a defined type, i.e. a `BMM_SIMPLE_TYPE` or `BMM_GENERIC_TYPE`.

The evaluation of inheritance relations defined in a BMM schema results in an acyclic graph such that ancestors and descendants can be visualised for any class. The following screen shot shows the ancestors view of a class `OBSERVATION`.

[.text-center]
.BMM class - ancestors view
image::images/awb_class_ancestors.png[id=awb_class_ancestors, align="center", width="50%"]
...
image::images/awb_multiple_inheritance.png[id=awb_multiple_inheritance, align="center", width="50%"]

Inheritance between generic classes ...

[.text-center]
.Generic inheritance
image::images/awb_generic_inheritance.png[id=awb_generic_inheritance, align="center", width="60%"]
...
include::{uml_export_dir}/classes/bmm_generic_class_effective.adoc[leveloffset=+1]
