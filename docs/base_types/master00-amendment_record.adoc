...
|[[latest_issue]]1.3.6
| Define the `TEMPLATE_ID` identifier: the human-readable identifier (HRID) form is strongly recommended and, when used, is the modern (ADL 2) form carrying a mandatory namespace; a legacy free-text name remains permitted but is deprecated. Adds `is_hrid()`, HRID segment/version accessors and a Template Identifiers section ({spec_tickets}/SPECBASE-42[SPECBASE-42^], {spec_tickets}/SPECPR-124[SPECPR-124^]).
|S Iancu
|[[latest_issue_date]]24 Jul 2026

|1.3.5
| Redefine `ARCHETYPE_ID` as an exact archetype identifier rather than an identifier-or-reference: `_value_` is now the grammar-validated `Hrid` primitive, its version carries the three-part semantic version, and `namespace`, `major_version`, `minor_version` and `patch_version` accessors are added ({spec_tickets}/SPECBASE-35[SPECBASE-35^], {spec_tickets}/SPECBASE-49[SPECBASE-49^], {spec_tickets}/SPECBASE-50[SPECBASE-50^], {spec_tickets}/SPECPR-363[SPECPR-363^]).
|S Iancu
|19 Jul 2026

|1.3.4
| Retarget the Syntaxes-section deferral for the human-readable identifier grammar to the new BASE _Human-Readable Identifiers_ specification ({spec_tickets}/SPECBASE-50[SPECBASE-50^]).
|S Iancu
|14 Jul 2026

|1.3.3
| Defer the archetype/template human-readable identifier grammar in the Syntaxes section to its canonical definition in Foundation Types (the `Hrid` primitive) ({spec_tickets}/SPECBASE-49[SPECBASE-49^]).
|S Iancu
|14 Jul 2026
