...
a|True if there is any overlap between intervals represented by Current and `_other_`, i.e. at least one limit of `_other_` is strictly inside the limits of this interval.
...
a|True if current interval properly contains `_other_`, i.e. all points of `_other_` are inside the current interval.
