...
h|*Inherit*
2+|`xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]`, `xref:{class_bmm_model_metadata}[BMM_MODEL_METADATA]`
...
h|*0..1*
|*class_definitions*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_bmm_class}[BMM_CLASS]>`
...
h|*1..1*
|*model_id* (): `xref:{class_string}[String]`
...
a_name: `xref:{class_string}[String][1]` +
): `xref:{class_bmm_enumeration}[BMM_ENUMERATION]`
a|Retrieve the enumeration definition corresponding to `_a_type_name_`.

h|*0..1*
|*primitive_types* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
a|List of keys in `_class_definitions_` of items marked as primitive types.

h|*0..1*
|*enumeration_types* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
a|List of keys in `_class_definitions_` of items that are enumeration types.

h|*1..1*
|*property_definition* (): `xref:{class_bmm_property}[BMM_PROPERTY]`
...
h|*1..1*
|*property_definition_at_path* (): `xref:{class_bmm_property}[BMM_PROPERTY]`
...
h|*1..1*
|*any_class_definition* (): `xref:{class_bmm_simple_class}[BMM_SIMPLE_CLASS]`
...
h|*1..1*
|*any_type_definition* (): `xref:{class_bmm_simple_type}[BMM_SIMPLE_TYPE]`
a|
|===
