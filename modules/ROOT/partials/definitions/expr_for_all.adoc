...
h|*1..1*
|*condition*: `xref:BASE:expression:core_package.adoc#_assertion_class[ASSERTION]`
a|Boolean condition that returns True or False when applied to a member of the operand of a `for_all` operator.

h|*1..1*
|*operand*: `xref:BASE:expression:core_package.adoc#_expr_value_ref_class[EXPR_VALUE_REF]`
