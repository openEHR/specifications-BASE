...
h|*0..1*
|*bmm_schema*: `xref:{class_bmm_schema}[BMM_SCHEMA]`
a|Persistent form of model.

h|*0..1*
|*bmm_model*: `xref:{class_bmm_model}[BMM_MODEL]`
...
h|*1..1*
|*meta_data*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_string}[String]>`
...
h|*0..1*
|*includes*: `xref:{class_list}[List]<xref:{class_string}[String]>`
...
h|*1..1*
|*is_top_level* (): `xref:{class_boolean}[Boolean]`
a|True if this is a top-level schema, i.e. not included by some other schema.

h|*1..1*
|*is_bmm_compatible* (): `xref:{class_boolean}[Boolean]`
