...
h|*Inherit*
2+|`xref:{class_bmm_package_container}[BMM_PACKAGE_CONTAINER]`
...
h|*0..1*
|*classes*: `xref:{class_list}[List]<xref:{class_bmm_class}[BMM_CLASS]>`
...
h|*0..1*
|*root_classes* (): `xref:{class_list}[List]<xref:{class_bmm_class}[BMM_CLASS]>`
