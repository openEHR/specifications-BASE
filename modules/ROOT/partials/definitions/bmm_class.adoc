...
h|*Inherit*
2+|`xref:{class_bmm_entity}[BMM_ENTITY]`, `xref:{class_bmm_model_element}[BMM_MODEL_ELEMENT]`
...
h|*1..1*
|*package*: `xref:{class_bmm_package}[BMM_PACKAGE]`
a|Package this class belongs to.

h|*0..1*
|*properties*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_bmm_property}[BMM_PROPERTY]>`
...
h|*0..1*
|*immediate_descendants*: `xref:{class_list}[List]<xref:{class_bmm_class}[BMM_CLASS]>`
...
h|*0..1*
|*all_ancestors* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
a|List of all inheritance parent class names, recursively.

h|*0..1*
|*all_descendants* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
...
h|*0..1*
|*suppliers_non_primitive* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
a|Same as `suppliers` minus primitive types, as defined in input schema.

h|*0..1*
|*supplier_closure* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
...
h|*1..1*
|*package_path* (): `xref:{class_string}[String]`
a|Fully qualified package name, of form: `package.package`.

h|*1..1*
|*class_path* (): `xref:{class_string}[String]`
a|Fully qualified class name, of form: `package.package.CLASS` with package path in lower-case and class in original case.

h|*0..1*
|*flat_properties* (): `xref:{class_list}[List]<xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_bmm_property}[BMM_PROPERTY]<xref:{class_bmm_type}[BMM_TYPE]>>>`
...
h|*1..1 +
(abstract)*
|*type* (): `xref:{class_bmm_defined_type}[BMM_DEFINED_TYPE]`
