...
h|*0..1 +
(redefined)*
|*bmm_type*: `xref:{class_bmm_container_type}[BMM_CONTAINER_TYPE]`
...
h|*1..1*
|*type_ref* (): `xref:{class_p_bmm_base_type}[P_BMM_BASE_TYPE]`
a|The target type; this converts to the first parameter in `_generic_parameters_` in `BMM_GENERIC_TYPE`. Persisted attribute.
|===
