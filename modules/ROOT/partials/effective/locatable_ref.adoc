...
2+|Inherits: xref:{class_object_ref}[OBJECT_REF]

2+h|*Attributes*

|*namespace*: `xref:{class_string}[String]` [1..1]
...
_Inherited from xref:{class_object_ref}[OBJECT_REF]_
...
|*id*: `xref:{class_uid_based_id}[UID_BASED_ID]` [1..1]
a|Globally unique id of an object, regardless of where it is stored.

|*path*: `xref:{class_string}[String]` [0..1]
...
|*as_uri* (): `xref:{class_string}[String]` [1..1]
