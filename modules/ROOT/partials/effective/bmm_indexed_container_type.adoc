...
2+|Inherits: xref:{class_bmm_definitions}[BMM_DEFINITIONS], xref:{class_bmm_entity}[BMM_ENTITY], xref:{class_bmm_type}[BMM_TYPE], xref:{class_bmm_container_type}[BMM_CONTAINER_TYPE]
...
_Inherited from xref:{class_bmm_container_type}[BMM_CONTAINER_TYPE]_
...
|*index_type*: `xref:{class_bmm_simple_type}[BMM_SIMPLE_TYPE]` [1..1]
