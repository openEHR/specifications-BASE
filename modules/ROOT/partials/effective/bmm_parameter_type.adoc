...
2+|Inherits: xref:{class_bmm_definitions}[BMM_DEFINITIONS], xref:{class_bmm_entity}[BMM_ENTITY], xref:{class_bmm_type}[BMM_TYPE], xref:{class_bmm_unitary_type}[BMM_UNITARY_TYPE]
...
|*type_constraint*: `xref:{class_bmm_defined_type}[BMM_DEFINED_TYPE]` [0..1]
...
|*effective_base_class* (): `xref:{class_bmm_class}[BMM_CLASS]` [1..1]
...
|*flattened_conforms_to_type* (): `xref:{class_string}[String]` [1..1]
a|Result is either `_conforms_to_type_` or `_inheritance_precursor.flattened_conforms_to_type_`.

|*effective_conforms_to_type* (): `xref:{class_bmm_class}[BMM_CLASS]` [1..1]
