...
2+|Inherits: xref:{class_bmm_definitions}[BMM_DEFINITIONS], xref:{class_bmm_entity}[BMM_ENTITY], xref:{class_bmm_type}[BMM_TYPE], xref:{class_bmm_unitary_type}[BMM_UNITARY_TYPE], xref:{class_bmm_defined_type}[BMM_DEFINED_TYPE]
...
|*base_class*: `xref:{class_bmm_generic_class}[BMM_GENERIC_CLASS]` [1..1]
a|The target type; this converts to the first parameter in `_generic_parameters_` in `BMM_GENERIC_TYPE`.

|*generic_parameters*: `xref:{class_list}[List]<xref:{class_bmm_unitary_type}[BMM_UNITARY_TYPE]>` [1..1]
...
|*effective_base_class* (): `xref:{class_bmm_generic_class}[BMM_GENERIC_CLASS]` [1..1]
...
|*is_partially_closed* (): `xref:{class_boolean}[Boolean]` [1..1]
