...
|_(abstract)_ *is_abstract* (): `xref:{class_boolean}[Boolean]` [1..1]
...
|_(abstract)_ *effective_base_class* (): `xref:{class_bmm_class}[BMM_CLASS]` [1..1]
a|Effective underlying class for this type, abstracting away any container type. +
_Inherited from xref:{class_bmm_type}[BMM_TYPE]_

|_(abstract)_ *type_name* (): `xref:{class_string}[String]` [1..1]
a|Formal string form of the type as per UML. +
_Inherited from xref:{class_bmm_type}[BMM_TYPE]_

|_(abstract)_ *has_subtypes* (): `xref:{class_boolean}[Boolean]` [1..1]
a|Determine if there are any type substitutions. +
_Inherited from xref:{class_bmm_type}[BMM_TYPE]_

|_(abstract)_ *subtypes* (): `xref:{class_list}[List]<xref:{class_string}[String]>` [1..1]
a|List of type substitutions if any available for this type within the current BMM model. +
_Inherited from xref:{class_bmm_type}[BMM_TYPE]_

|_(abstract)_ *flattened_type_list* (): `xref:{class_list}[List]<xref:{class_string}[String]>` [1..1]
