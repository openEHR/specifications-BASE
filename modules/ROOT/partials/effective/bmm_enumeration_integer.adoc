...
2+|Inherits: xref:{class_bmm_definitions}[BMM_DEFINITIONS], xref:{class_bmm_definitions}[BMM_DEFINITIONS], xref:{class_bmm_entity}[BMM_ENTITY], xref:{class_bmm_model_element}[BMM_MODEL_ELEMENT], xref:{class_bmm_class}[BMM_CLASS], xref:{class_bmm_simple_class}[BMM_SIMPLE_CLASS], xref:{class_bmm_enumeration}[BMM_ENUMERATION]
...
|*underlying_type_name*: `xref:{class_string}[String]{nbsp}={nbsp}"INTEGER"` [1..1]
...
_Inherited from xref:{class_bmm_enumeration}[BMM_ENUMERATION]_
